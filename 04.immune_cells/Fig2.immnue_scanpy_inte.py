...
adata = anndata.read_loom('../../all_immnue.loom')
...
obs = pd.read_csv('/dellfsqd2/ST_OCEAN/USER/hankai/Project/18.lamprey_atlas/07.lamprey_atlas/lamprey.obs.txt', 
...
del_celltype1124 = ['Endothelial cell', 'Germ cell', 'Inhibitory telencephalon', 'Meningeal fibroblasts', 
        'NKTcell', 'Schwann cell', 'Stromal cell', 'keratinocyte']
adata = adata[~adata.obs['celltype.1124'].isin(del_celltype1124)]
del_proliferating_tissues = ['dorsal_fin', 'gill', 'heart', 'muscle', 'skin', 'tailfin']
adata = adata[~((adata.obs['celltype.1124'] == 'Proliferating cell') & (adata.obs['tissue'].isin(del_proliferating_tissues)))]
...
celltype2color = pd.read_csv('/dellfsqd2/ST_OCEAN/USER/hankai/Project/18.lamprey_atlas/07.lamprey_atlas/celltype2color.1124.txt', 
        sep='\t', header=0)
celltype2color = dict(celltype2color.to_dict('tight')['data'])
tissue2color = pd.read_csv('/dellfsqd2/ST_OCEAN/USER/hankai/Project/18.lamprey_atlas/07.lamprey_atlas/tissue2color.txt', 
        sep='\t', header=0)
